  ...
  'change #assetCloud': function(evt, templ) {
    ...
    templ.uploader.send(file, function (error, downloadUrl) {
      if (error) {
        ...
        $.post(
          Meteor.settings.public.resizeUrl + '/get-serving-url',
          {url: downloadUrl}
        ).then(res => {

          $('#filename').text(file.name);
          $('#assetType').val(file.type);
          $('#assetFile').val(res.serving_url);

          if (file.type.indexOf('image') > -1) {
            var pic_real_width, pic_real_height;
            $("<img/>") // Make in memory copy of image to avoid css issues
              .attr("src", res.serving_url)
              .load(function () {
...
Template.assetCloudUpload.helpers({
  ...
  isImage: function() {
    ...
    var assetType = currentAsset.assetType;
    if (!assetType) return;

    return currentAsset.assetType.indexOf('image') > -1;
  },
  ...
  isAudio: function() {
    ...
    return currentAsset.assetType.indexOf('audio') > -1
  }
});
