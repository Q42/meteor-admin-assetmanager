...
Template.registerHelper('bbox', function(assetId, s) {
   return _resize(assetId, s, s, false);
});


/*/

 _resize(assetId, width, height)
 _resize(assetId, width, height, cover)
...
function _resize(assetId, width, height, cover) {
    //console.log('resize', assetId, width, height, cover == 'cover' ? 'cover': 'contain');
    var asset = AssetCollection.findOne(assetId);
    if (!asset) return;

    var assetUrl = AssetManager.createAbsoluteUrl(asset.file);

    if (!Meteor.settings.public || !Meteor.settings.public.resizeUrl) return assetUrl;
    if (!asset.imageWidth || !asset.imageHeight) return assetUrl;
    ...
    return Meteor.settings.public.resizeUrl +  '/resize?url=' + assetUrl + '&s=' + Math.round(boundingBox);
}
